...
#SBATCH --error=err/griderr_ntuple_photonsel.sub01.%A.%a.node%N.err
#SBATCH --output=log/stdout_ntuple_photonsel.sub01.%A.%a.node%N.log
...
VALSCRIPT=${GEN2NTUPLE_DIR}/tufts_run_ntuple_maker_photon_selection.sh
...
apptainer exec --bind ${BINDINGS} ${CONTAINER} bash -c "cd ${GEN2NTUPLE_DIR} && source ${VALSCRIPT} ${RECOFILELIST} ${TRUTHFILELIST} ${WEIGHTFILE} ${OUTTAG} ${NFILES} ${SAMPLENAME}"
